...
def main():
    typeCount = 29
    days = 30
    data = getData(typeCount)
    if checkByMon(data):
        R = genR(data)
        CJ = genCJ(data, R)
        data['ruKu'] = R
        data['chouJian'] = CJ
        monPlan = genPlan(data, typeCount)
        monPlan = adjustPlan(monPlan, typeCount, days, data)
        output = genOutput(monPlan, days)
